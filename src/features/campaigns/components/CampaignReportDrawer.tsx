...
import { Campaign, Contact, SystemLog, Template } from "@/shared/context/AppContext";
...
export const CampaignReportDrawer: React.FC<CampaignReportDrawerProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<"analytics" | "setup">(initialTab);
  ...
  // Resolve the template used so the read-only Setup tab can show its full payload.
  const setupTemplate = templates.find((t) => t.name === campaign.templateName) || null;

  // Filter contacts matching campaign target tag or dynamic segment rules
  let campaignContacts: Contact[] = [];
  if (campaign.segment && (campaign.segment as any).rules) {
    const rules = (campaign.segment as any).rules;
    campaignContacts = contacts.filter((c) => {
      // Evaluate AND rules
      const passAll = !rules.all ? true : rules.all.every((rule: any) => {
        if (rule.field === "tags") {
          const tags = Array.isArray(rule.value) ? rule.value : String(rule.value || "").split(",").map(t => t.trim()).filter(Boolean);
          if (tags.length === 0) return true;
          if (rule.op === "in") return tags.some((t: string) => c.tags.includes(t));
          if (rule.op === "neq") return !tags.some((t: string) => c.tags.includes(t));
          return tags.every((t: string) => c.tags.includes(t));
        }
        if (rule.field === "status") {
          return rule.op === "neq" ? c.status !== rule.value : c.status === rule.value;
        }
        if (rule.field === "source") {
          const sValue = String(rule.value || "").toLowerCase();
          const cSource = String(c.source || "").toLowerCase();
          return rule.op === "contains" ? cSource.includes(sValue) : cSource === sValue;
        }
        return true;
      });

      // Evaluate OR rules
      const passAny = !rules.any ? true : rules.any.some((rule: any) => {
        if (rule.field === "tags") {
          const tags = Array.isArray(rule.value) ? rule.value : String(rule.value || "").split(",").map(t => t.trim()).filter(Boolean);
          if (tags.length === 0) return true;
          if (rule.op === "in") return tags.some((t: string) => c.tags.includes(t));
  ...
  } else if (campaign.targetTag === "all") {
    campaignContacts = contacts;
  } else if (campaign.targetTag) {
    campaignContacts = contacts.filter((c) => c.tags.includes(campaign.targetTag));
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/40 backdrop-blur-sm transition-opacity duration-300">
      ...
      <div className="relative w-full max-w-2xl bg-white h-full shadow-xl flex flex-col z-10 animate-slide-left border-l border-stone-200 overflow-hidden">
        ...
        {/* Tab Switcher */}
        <div className="flex items-stretch border-b border-stone-200 shrink-0 bg-white">
          <button
            onClick={() => setActiveTab("analytics")}
            className={`flex items-center gap-1.5 px-5 py-3 text-[11px] font-bold uppercase tracking-wider border-b-2 transition-colors cursor-pointer ${
              activeTab === "analytics"
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8 bg-[#fafaf9]">
          ...
          {activeTab === "setup" ? (
            <div className="space-y-6">
              {/* Read-only notice */}
              <div className="flex items-center gap-2 bg-stone-100 border border-stone-200 px-4 py-2.5 rounded-none text-[11px] text-stone-600 font-semibold">
              ...
              <div className="bg-white rounded-none border border-stone-200 kc-float overflow-hidden flex flex-col p-6 space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-stone-50 border border-stone-200 rounded-none">
                    <div className="text-[10px] uppercase font-bold text-stone-400 mb-1">Status</div>
                    <span className={`inline-flex text-[10px] font-bold uppercase px-2.5 py-0.5 rounded-none border ${getStatusColor(campaign.status)}`}>
                      {campaign.status === "PendingTemplate" ? "Awaiting Approval" : campaign.status}
                    </span>
                  </div>
                  ...
                  <div className="p-4 bg-stone-50 border border-stone-200 rounded-none">
                    ...
                    <div className="text-sm text-stone-900 font-semibold">{sent} contact(s)</div>
                  </div>

                  {/* Target */}
                  <div className="p-4 bg-stone-50 border border-stone-200 rounded-none sm:col-span-2">
                    <div className="text-[10px] uppercase font-bold text-stone-400 mb-1">
                      {campaign.segment ? "Target Segment" : "Target Tag"}
                    </div>
                    <div className="text-sm text-stone-900 font-semibold">
                      {campaign.segment ? (campaign.segment as any).name : campaign.targetTag || "All contacts"}
                    </div>
                    {campaign.segment && (
                      <div className="text-[10px] text-stone-500 mt-1">Currently resolves to {campaignContacts.length} contact(s).</div>
                    )}
                    {campaign.excludeTag && (
                      <div className="text-[10px] text-stone-500 mt-1">Excludes tag: <span className="font-bold text-stone-700">{campaign.excludeTag}</span></div>
                    )}
                  </div>
                  ...
                  {/* Anti-spam delay */}
                  <div className="p-4 bg-stone-50 border border-stone-200 rounded-none">
                    <div className="text-[10px] uppercase font-bold text-stone-400 mb-1">Anti-Spam Delay</div>
                    <div className="text-sm text-stone-900 font-semibold">{campaign.delay ?? 1}s between messages</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-none border border-stone-200 kc-float overflow-hidden flex flex-col p-6 space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="text-sm text-stone-900 font-bold">{campaign.templateName || "—"}</div>
                  {setupTemplate?.category && (
                    <span className="text-[9px] uppercase tracking-wider text-stone-400 font-bold">{setupTemplate.category}</span>
                  )}
                </div>
                ...
                {setupTemplate ? (
                  <>
                    {(campaign.mediaType && campaign.mediaType !== "none") && (
                      <div className="flex items-center gap-1.5 text-[10px] text-stone-500 font-semibold uppercase">
                        <ImageIcon className="w-3.5 h-3.5" />
                        {campaign.mediaType} header
                      </div>
                    )}
                    <div className="bg-stone-50 border border-stone-200 rounded-none p-4 text-xs text-stone-800 font-medium leading-relaxed whitespace-pre-wrap">
                      {setupTemplate.body}
                    </div>
                    {setupTemplate.buttons && setupTemplate.buttons.length > 0 && (
                      <div className="space-y-1.5">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {setupTemplate.buttons.map((btn, i) => (
                            <span key={i} className="bg-white border border-stone-300 rounded-none px-3 py-1.5 text-[10px] font-bold text-stone-700">
                              {btn}
                            </span>
                          ))}
                        </div>
                  ...
                  <div className="text-[11px] text-stone-500 italic">Template body is not available in this workspace view.</div>
                )}
                ...
                {campaign.variables && campaign.variables.length > 0 && (
                  <div className="space-y-1.5">
                    <div className="text-[10px] uppercase font-bold text-stone-400 flex items-center gap-1">
                      <Hash className="w-3 h-3" /> Personalization Variables
                    </div>
                    <div className="space-y-1">
                      {campaign.variables.map((v, i) => (
                        <div key={i} className="flex items-center justify-between bg-stone-50 border border-stone-200 px-3 py-1.5 text-[11px]">
                          <span className="font-bold text-stone-700">{`{{${i + 1}}}`} {v.key ? `· ${v.key}` : ""}</span>
                          <span className="text-stone-500">{v.type === "contact_field" ? `Contact field: ${v.value}` : v.value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <>
              ...
              <div className="bg-white rounded-none border border-stone-200 kc-float overflow-hidden flex flex-col">
                ...
                <div className="flex border-b border-stone-200 bg-stone-50 shrink-0">
                  ...
                  >
                    Audience ({campaignContacts.length})
                  </button>
                  <button
